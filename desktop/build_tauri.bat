@echo off
REM Sentinel SMDL desktop — release build (Windows).
REM
REM Pattern mirrors sentinel-watchdog/admin/build_tauri.bat exactly,
REM because Git Bash's /usr/bin/link.exe shadows MSVC link.exe and the
REM vcvars64 priming below is the only reliable workaround. See memory
REM `feedback_tauri_windows_bash_link_shadow`.
REM
REM Outputs:
REM   src-tauri\target\release\sentinel-smdl-desktop.exe   (bare exe)
...
taskkill /F /IM sentinel-smdl-desktop.exe >NUL 2>&1
